...
const userSchema = new Schema(
    {
        ...
        avatar: {
            type: String, // cloudinary url use krenge: its like aws but free
            required: false
        },
        coverImage: {
            type: String, // cloudinary url use krenge: its like aws but free
...
// 	•	It takes next as a parameter to move to the next mi ddleware in the chain.
...
userSchema.methods.generateAccessToken = function(){ //This function generateAccessToken is a method of the userSchema in a MongoDB-based Node.js application using JWT (JSON Web Token).generates a JWT token for a user when called.
    return jwt.sign(
        {
            _id: this.id,
            email: this.email,
